...
- **IDE diagnostics in this harness are routinely stale** (RED-phase snapshots, worktree files). Trust `bunx tsc --noEmit` and `bun run lint`, not the squiggles. The old accepted-debt hints (`generateObject`, `FormEvent`, lockfile name) are FIXED — a deprecation warning appearing for one of those is a real regression now, not shruggable debt.
