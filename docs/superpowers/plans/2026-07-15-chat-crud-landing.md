...
- Unit tests run against real Postgres (`bun run test`); e2e `bun run test:e2e` (19 tests must stay green; this phase adds more). Commits: gitmoji, one behavior each, tests ride with their behavior.
...
- `src/lib/conversations.ts`:
  ...
  - `loadMessageTree(conversationId, userId): Promise<{ messages: LoadedMessage[]; activeLeafId: string | null }>` — ALL rows (flat, `(createdAt, id)` asc, decrypted, corrupt-row skip), same fields as loadMessages rows.
...
- **Regenerate:** verify the target via `loadMessageTree` (owner check happens inside) — must exist and `sender === "ai"`, else `NotFoundError`; context = the active-path-style chain ENDING AT ITS PARENT: compute with `resolveActivePath(treeNodes, target.parentId)` (parent null ⇒ empty context is invalid — a root AI message can't exist; treat as 404). Build `uiMessages` from that chain; skip risk classification (`riskLevel = "none"` for headers); DO NOT save a client message. In persistence, the AI row saves with `parentId: target.parentId`.
