...
**Addendum (user, 2026-07-09, with screenshot):** make crises more VISIBLE in the chat layout itself, not just the small badge. Locked treatment (therapist reading view): a crisis message's bubble gets an unmistakable-but-calm treatment — warm amber left border + soft amber-tinted background wash + the existing Crisis pill moved INTO the bubble's header line (not floating below), consistent with the attention queue's warm-urgency language; never alarm-red. The navigator (above) and this treatment ship together. CLIENT view stays quiet by design — a client's own crisis moments shouldn't be loudly re-labeled at them every scroll-by (the crisis card at send time is their surface); this is a deliberate product stance, reversible if the user disagrees.
