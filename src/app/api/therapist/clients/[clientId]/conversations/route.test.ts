...
describe("GET /api/therapist/clients/[clientId]/conversations", () => {
  ...
  it("lists granted conversations for a linked client", async () => {
    const clientId = `test-${randomUUID()}`;
    const therapistId = `test-${randomUUID()}`;
    const { token } = await createInvite(clientId, "client");
    await acceptInvite(token, therapistId);
    const conv = await createConversation(clientId, "Shared chat");
    ...
    expect(body[0].id).toBe(conv.id);
    expect(body[0].title).toBe("Shared chat");
  });
